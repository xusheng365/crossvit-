...
class VisionTransformer(nn.Cell):
    ...
    def _init_weights(self) -> None:
        for _, cell in self.cells_and_names():
            ...
            if isinstance(cell, nn.Dense):
                cell.weight.set_data(init.initializer(TruncatedNormal(sigma=.02), cell.weight.data.shape))
                if cell.bias is not None:
                    cell.bias.set_data(init.initializer(TruncatedNormal(sigma=.02), cell.bias.shape))
            elif isinstance(cell, nn.LayerNorm):
                cell.gamma.set_data(init.initializer(TruncatedNormal(sigma=.02), cell.gamma.shape))
                cell.beta.set_data(init.initializer(TruncatedNormal(sigma=.02), cell.beta.shape))
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_branches):
            ...
            # print(i, " yes6")
            # print("cls_tokens", cls_tokens.dtype)
            # print("tmp", tmp.dtype)
    ...
    def construct(self, x):
        ...
        # ce_logits = torch.mean(torch.stack(ce_logits, dim=0), dim=0) #拼接之后求平均
